...
public class SWEA_1247_최적경로2 {
    ...
    public static int getInstance(Point p1, Point p2){
    ...
    public static void dfs(int cnt, Point prev, int sum, int isVisit){
        ...
        if(cnt==n){
            // 마지막 고객과 집을 연결 
            int toHome = sum+getInstance(home, prev);
            // 최소값 갱신
        ...
        for(int i=0;i<n;i++){
            ...
            // 내가 갈 다음 연산을 위한 조정
            dfs(cnt+1, clients[i], sum+getInstance(prev, clients[i]), isVisit|1<<i);
        }
    }
}
